...
<script>
    ...
    export default {
        name: "bitbucketOauth",
        created(){
            this.getCallbackInfo();//拿code换token
            // this.getGitBindInfo();
        },
        data(){
            return{
        ...
        methods:{
            ...
            getCallbackInfo(){
                ...
                if(code !== null){
                    let param = {
                        code:code,
                        oauthType:4,//1 官方的gitlab 2 github 3 gitee 4 bitbucket
                    };
                    httpPost('/v1/authorization/coreGitOauth/bitbucket/insert', param).then(results => {
            ...
            getReposList(gitUserName) {
                ...
                httpGet("/v1/authorization/bitbucket/repos/list", {userName: gitUserName}).then(results => {
            ...
            beforeBindThirdGit(){
                ...
                if(this.gitName.indexOf("bitbucket")>=0){      //团队选择的是gitlab平台
                    data.gitProjectId = reposInfo.id;
                }
                data.reposName = reposInfo.slug;
                data.reposHttpUrl = reposInfo.httpsUrl;
                data.reposSshUrl = reposInfo.sshUrl;
                console.log(131, reposInfo);

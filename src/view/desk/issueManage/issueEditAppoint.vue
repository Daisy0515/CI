    ...
    <div class="container deskHeader">
      <el-card class="box-card0">
        ...
        <el-form :model="ruleForm2" ref="ruleForm2" class="demo-ruleForm">
          ...
          <el-button type="primary" @click="submitForm()" size="medium" style="width:150px;display:block;margin:0 auto">保存</el-button>
          </el-form-item>
        </el-form>
      </el-card>
...
created: function() {
  ...
  methods: {
    ...
    getIssue(){
      httpGet(`/v1/authorization/bug/edit/appoint?id=${this.id}`).then(
        results => {
     ...
     //获取某个项目的可指派人
    getUsers(){
      httpGet(`/v1/authorization/bug/edit/appointuser?id=${this.id}`).then(
        results => {

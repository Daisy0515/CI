<template>
	<div class="myTable">
		...
		<div class="header_top">
			...
			<router-link to="/testEmploy"><el-button type="primary">发布内测</el-button></router-link>
		</div>
		<el-table :data="bidTable" style="width:1200px;margin:0 auto" :header-cell-style="rowClass" v-loading="loading" >
			...
			<el-table-column label="操作" align="center" width="450" >
				<template slot-scope="scope">
					<router-link v-if="scope.row.status===3||scope.row.status===2" :to="{ path: 'myTestView', query: { id: scope.row.id } }">
						<i class="el-icon-search"></i>
						查看
					</router-link>
					<router-link v-if="scope.row.status===1" :to="{ path: 'myTestView&Edit', query: { id: scope.row.id } }">
...
<!-- <el-form :model="ruleForm" :rules="rules" ref="ruleForm" style="display:inline-block"> -->
					<el-popover placement="right" trigger="click" @hide="changeProgress(ruleForm)">
						<div>
						...
						<el-table :data="ruleForm" v-loading="loading1">
						    <el-table-column width="100" property="userName" label="用户名" align="center"></el-table-column>
						    <el-table-column width="100" property="school" label="学校" align="center"> <template slot-scope="scope"><span v-if="scope.row.school===null">暂无 </span></template> </el-table-column>
							...
							<el-table-column width="100" property="companyName" label="公司名" align="center"> <template slot-scope="scope"><span v-if="scope.row.school===null">暂无 </span></template> </el-table-column>
						  </el-table>
...
<!-- </el-form> -->
					<router-link v-if="scope.row.status===3||scope.row.status===2" :to="{path:'./testIssueSum',query:{id:scope.row.id}}"  >
...
export default {
	...
	methods: {
		...
		getTestUser(Id){
			...
			this.ruleForm="";
			this.ownNum=0;
			//get /v1/authorization/test/get/testuserinfo 
		...
		overTest(id) {
			MessageBox.confirm('是否确定将此测试结束？', '提示', {
				...
				.then(() => {
					//put /v1/authorization/test/teststatus/update 
					this.testId.id=id;
					//alert(this.testId.id);
		...
		getView(val = this.pageData) {
			this.loading = true;
			//get /v1/authorization/test/search/testinfo
			//alert(222);

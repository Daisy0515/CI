    ...
    <div class="ganttIndex" style="width:100%">
        <ul class="taskMain">
            <li class="main-index" v-for="(item, index) in gettaskList" :key="index">
				<span class="topBom">
					<i v-if="index != 0 && gettaskList.length != 1" @click="topBtb(index, item.id)"
                       class="icon iconfont icon-tubiaozhizuo-2"></i>
					<i v-if="index != gettaskList.length - 1" @click="botBtb(index, item.id)"
                       class="icon iconfont icon-tubiaozhizuo-1"></i>
				</span>
                ...
                <el-input v-show="item.changeTask" v-model="item.changeTitle" placeholder="请输入内容"
                          style="width:250px;margin-right:10px" size="small"></el-input>
                <el-button
                ...
                <el-popover placement="right" trigger="click" @hide="changeProgress(item.upData, item)">
                    <div class="popo-item">
                        <el-select v-model="item.upData.userId" slot="prepend" placeholder="请选择用户" size="small"
                                   style="width:100%">
                            <el-option v-for="(items, index) in item.userList" :key="index" :value="items.id"
                                       :label="items.userName"></el-option>
                        </el-select>
                    </div>
                    ...
                    <div class="popo-item">
                        <el-input placeholder="完成度" size="small"
        ...
        >
            <el-input type="textarea" :rows="10" placeholder="请输入描述"
                      v-model="gettaskList[dialogIndex].description"></el-input>
            <span slot="footer" class="dialog-footer">
				<el-button @click="dialogVisible = false" size="small">取 消</el-button>
				<el-button type="primary" @click="changeDescription(gettaskList[dialogIndex])"

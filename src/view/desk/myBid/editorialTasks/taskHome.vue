  ...
  <div class="ganttIndex" style="height:100%;width:100%">
      <ul class="taskMain">
        <li class="main-index" v-for="(item,index) in gettaskList" :key="index">
          <span class="topBom">
            <i
              v-if="index!=0&&gettaskList.length!=1"
              @click="topBtb(index,item.id)"
              class="icon iconfont icon-tubiaozhizuo-2"
            ></i>
            <i
              v-if="index!=gettaskList.length-1"
              @click="botBtb(index,item.id)"
              class="icon iconfont icon-tubiaozhizuo-1"
            ></i>
          </span>
          <span
            @click="dialogVisible = true,dialogIndex=index"
		 ...
		 <!-- 这里有问题 -->
          ...
          <el-button
            v-show="item.changeTask"
            size="small"
            @click="item.changeTask=!item.changeTask;item.changeTitle=item.content"
          >取消</el-button>
          <el-popover placement="right" trigger="click" @hide="changeProgress(item.upData,item)">
            <div class="popo-item">
              <el-select
                v-model="item.upData.userId"
                slot="prepend"
                placeholder="请选择用户"
                size="small"
                style="width:100%"
              >
                <el-option
                  v-for="(items,index) in item.userList"
                  :key="index"
                  :value="items.id"
            ...
            <div class="popo-item">
              <el-input
                placeholder="完成度"
                size="small"
                :disabled="item.upData.accomplishProgress===100?true:false"
                v-model="item.upData.accomplishProgress "
              >
            ...
            <el-button slot="reference" v-show="!item.changeTask" style="color:#909399">
              ...
              <span
                v-show="!item.select"
              >{{item.participantNames}}，{{item.startTime}} 至 {{item.endTime}}，完成度{{item.accomplishProgress}}%</span>
    ...
    >
      <el-input
        ...
        :rows="10"
        placeholder="请输入描述"
        v-model="gettaskList[dialogIndex].description"
      ></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false" size="small">取 消</el-button>
        <el-button
          type="primary"
          @click="changeDescription(gettaskList[dialogIndex])"
          size="small"
        >确 定</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    changeProgress(end, start) {
      ...
      if (
        new Date(end.startTime.replace(/\-/g, "/")) >
        new Date(end.endTime.replace(/\-/g, "/"))
      ) {
        ...
        let sustainTime =
          (endCompare - startCompare) / (1000 * 60 * 60 * 24) + 1;
    ...
    //任务上移
    topBtb(index, taskId) {
      httpPut("/v1/authorization/bids/ordersup/update", { id: taskId }).then(
        results => {
    ...
    //任务下移
    botBtb(index, taskId) {
      httpPut("/v1/authorization/bids/ordersdown/update", { id: taskId }).then(
        results => {
          const { httpCode, msg } = results.data;
          if (httpCode === 200) {
            this.setOrder({ index1: index + 1, index2: index });
    ...
    changeTaskName(endValue, startValue) {
      ...
      httpPut("/v1/authorization/bids/missioninfo/update", endValue).then(
        results => {
          const { httpCode, msg } = results.data;
          if (httpCode === 200) {
    ...
    changeDescription(value) {
      ...
      httpPut("/v1/authorization/bids/missioninfo/update", newValue).then(
        results => {
          const { httpCode, msg } = results.data;
          if (httpCode === 200) {
    ...
    deleTask(id, index) {
      ...
      MessageBox.confirm("是否确定将此任务删除？", "提示", {
        ...
        .then(() => {
          httpDelete(`/v1/authorization/bids/missioninfo/delete/${id}`).then(
            results => {
              const { httpCode, msg } = results.data;

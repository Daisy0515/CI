    ...
    <el-dialog title="已选择对象" :visible.sync="visible" width="80%" :before-close="closeSelectedUserDialog">
        <el-table :data="userList" v-loading="userListLoading">
            ...
            <el-table-column label="发送">
                <template slot-scope="scope">
                    <el-checkbox v-model="scope.row.isRemind"></el-checkbox>
                </template>
            </el-table-column>
        </el-table>
        ...
        methods: {
            ...
            getEmailContentInfo(row) {
                ...
                if (row.emailConfig === null) {//定制信件框没有保存过内容
                    ...
                    httpPost("/v1/authorization/review/expertemail/update", data).then(results => {
            ...
            sendAllEmail() {
                ...
                for(let item of this.userList){
                    if(item.isRemind){
                        previewList.push(deepCopyObject(item));
                    }
                }
                let data = {
                ...
                httpPost("/v1/authorization/review/expertemailall/send", data).then(results => {
            ...
            previewEmail() {
                ...
                httpPost("/v1/authorization/review/expertemailpreview/get", data).then(results => {
                    const {httpCode, msg, data} = results.data;
                    if (httpCode === 200) {

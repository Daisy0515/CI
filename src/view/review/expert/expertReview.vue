	...
	<div>
		...
		<el-table v-loading="loading" :data="tableData" style="width:1000px;margin:20px auto" :header-cell-style="rowClass">
			...
			<el-table-column prop="accomplishProgress" label="操作" align="center" width="220px">
				<template slot-scope="scope">
					<el-button @click="handleClickDetail(scope.row.reviewId)" type="text" size="medium"><i class="el-icon-search"></i>查看详情</el-button>
					<el-button @click="handleEvaluate(scope.row)" type="text" size="medium"><i class="el-icon-edit"></i>评价
					</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			searchList() {
				if (this.searchData.tpye == "") {
					this.searchData.type == null
				}
				if (this.searchData.submitTimeStart == "") {
					this.searchData.submitTimeStart == null
				}
				if (this.searchData.submitTimeEnd == "") {
					this.searchData.submitTimeEnd == null
				}
			...
			getView(val = this.pageData) {
				if (this.pageData.tpye == "") {
					this.pageData.type == null
				}
				if (this.pageData.submitTimeStart == "") {
					this.pageData.submitTimeStart == null
				}
				if (this.pageData.submitTimeEnd == "") {
					this.pageData.submitTimeEnd == null
				}
